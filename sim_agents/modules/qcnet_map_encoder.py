"""
Based on https://github.com/ZikangZhou/QCNet/blob/main/modules/qcnet_map_encoder.py
"""
...
from sim_agents.utils import merge_edges
    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        ...
        if self.dataset == 'waymo':
            if self.input_dim == 2:
                ...
                x_pt = pos_pt[:, 2].unsqueeze(-1)
                x_pl = pos_pl[:, 2].unsqueeze(-1)
            else:
        ...
        edge_index_pl2pl_knn = knn_graph(
            x=pos_pl[:, :2], k=self.pl2pl_knn, loop=False,
            batch=data['map_polygon']['batch'] if isinstance(data, Batch) else None
        )
        type_pl2pl = data['map_polygon', 'to', 'map_polygon']['type']
        type_pl2pl_knn = type_pl2pl.new_zeros(edge_index_pl2pl_knn.size(1), dtype=torch.uint8)
        edge_index_pl2pl, type_pl2pl = merge_edges(edge_indices=[edge_index_pl2pl_knn, edge_index_pl2pl],
                                                   edge_attrs=[type_pl2pl_knn, type_pl2pl], reduce='max')
